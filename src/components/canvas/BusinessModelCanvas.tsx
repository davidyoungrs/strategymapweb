...
const BmcCell: React.FC<BmcCellProps> = ({ 
  ...
  <div className={`p-6 border border-zinc-100 dark:border-zinc-800 flex flex-col group transition-all hover:bg-white dark:hover:bg-zinc-900/50 hover:shadow-xl hover:shadow-zinc-200/50 dark:hover:shadow-black/50 ${className}`}>
    <div className="flex items-center gap-3 mb-4">
      <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${isMain ? 'bg-blue-600 text-white' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 group-hover:text-blue-600'}`}>
        {icon}
      </div>
      <div>
        <h3 className={`text-[10px] font-black uppercase tracking-[0.2em] leading-none ${isMain ? 'text-blue-600 dark:text-blue-400' : 'text-zinc-900 dark:text-zinc-100'}`}>{title}</h3>
    ...
    <textarea
      ...
      className={`flex-1 w-full bg-transparent resize-none outline-none text-sm leading-relaxed placeholder:text-zinc-300 dark:placeholder:text-zinc-800 font-medium ${isMain ? 'text-zinc-900 dark:text-zinc-100 font-bold' : 'text-zinc-600 dark:text-zinc-400'}`}
